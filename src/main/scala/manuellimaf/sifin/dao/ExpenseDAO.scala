...
object ExpenseDAO extends DBConnectionSupport {
  ...
  def getMonthExpenses(month: Month): Seq[Expense] =
    withQueryResult(
      """select e.id, e.amount, e.day, e.description, curr.id, curr.symbol, curr.code, pm.id, pm.name, pm.type, pm.description,
    ...
    res => Expense(id = Some(res.getLong("e.id")),
      amount = res.getDouble("e.amount"),
      day = res.getInt("e.day"),
      month = month,
      description = res.getString("e.description"),
      ...
      category = Category(id = res.getLong("cat.id"),
        ...
        categoryType = res.getString("cat.type"))
    )
  }
